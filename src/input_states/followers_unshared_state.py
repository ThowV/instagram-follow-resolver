...
class FollowersUnsharedInputState(AbstractInputState):
    def on_enter(self, prefix, curr_input_state, client):
        ...
        if unshared:
            ...
            if response == 1:
                # We used the previously stored data
                return curr_input_state, client

        # We are not using the previously stored data
        # Hence we clear it and start retrieval of new data
        unshared.clear()
    ...
    def list_info(self):
        self.console.print("\nFound {} people who don't follow you back!".format(len(unshared)), style="light_salmon3")
